...
const createAsset = async (req,res) => {
    ...
    const response = await mongodb.getDatabase().db().collection('asset').insertOne(asset);
    if (response.acknowledged) {
        res.status(204).send();    
    }   else {
        res.status(500).json(response.error || 'Some error occurred while updating the asset.');
    }
}
...
const updateAsset = async (req,res) => {
    ...
    const response = await mongodb.getDatabase().db().collection('asset').replaceOne({_id: userId}, asset);
    if (response.acknowledged) {
        res.status(204).send();    
    }   else {
        res.status(500).json(response.error || 'Some error occurred while updating the asset.');
...
const deleteAsset = async (req,res) => {
    ...
    swagger.tags=['Assets']
    const userId= new ObjectId(req.params.id);
    const response = await mongodb.getDatabase().db().collection('asset').deleteOne({_id: userId});
    if (response.acknowledged) {
        res.status(204).send();    
    }   else {
        res.status(500).json(response.error || 'Some error occurred while deleting the asset.');
    }
}

const getAssetByOwner = async (req,res) => {
    swagger.tags=['Assets']
    const result = await mongodb.getDatabase().db().collection('asset').find({assetOwner: req.params.assetOwner});
    result.toArray().then((asset)=> {
        res.setHeader('Content-Type', 'application/json');
        res.status(200).json(asset);
    });
